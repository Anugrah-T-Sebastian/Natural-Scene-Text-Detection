...
Now let us try to using Tesseract OCR for natural scene text detection.
...
4. Create a separate original copy and clone copy for the future purposes. Save the tesseract configuration in the cus_config variable as per the required page segment mode. Let us chose the page segmentation mode 6 (psm 6) i.e. “Assume a single uniform block of text”, as it gives the best results for our chosen example.

![Image](OCRimages/img3.jpg)

The above output shows that the Tesseract OCR failed to detection and recognize lot o text present in the image. One of the basic performance improvement methods while using an OCR is to convert the image into Grayscale before passing the it into the OCR.
5. Let us try few more preprocessing methods such as:-
*	Greyscaling
*	Thresholding
*	Eroding
...
We can see that performance of Tesseract OCR is not very good for natural scene text detection. This mainly due to the lack of constrained examples, since Tesseract gives the best results for constrained examples.
One way of improving performance is better detection of the text region in the images. Thankfully, we have EAST algorithm (Efficient and Accurate Scene Text) which gives very good performance in detecting text regions in the image.
...
With EAST algorithm we can see that our text regions are better detected and compared to the Plain Pytesseract OCR. The co-ordinates of the text region in the image are stored in the roi list. Let us pass on the regions of interest to the Pytesseract OCR for recognition.

![Image](OCRimages/img6.jpg)

We can see that the Pytesseract OCR has recognized majority of the text regions correctly. The recognition text can be written into text editing file or databases, and can be used for a variety of different purposes.
...
In the project we also saw the limitations exhibited by the Tesseract OCR. And improvements in performance when it is combined with better text detection and segmentation algorithms.
